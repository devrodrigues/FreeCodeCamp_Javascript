...
of a valid US number. The following are examples of valid formats for US 
...
8oo-six427676;laskdjf. Your job is to validate or reject the US phone number based on 
...
function telephoneCheck(str) {
  const isValidPhone = /^1?((?<=1)\s)?(\((?=[\d]{3}\)))?[\d]{3}((?<=\([\d]{3})\))?[\s-]?[\d]{3}[\s-]?[\d]{4}$/;
  return isValidPhone.test(str);
}
